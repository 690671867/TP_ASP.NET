...
using TpASPGestionCours.Filters;
using TpASPGestionCours.Models;
        ...
        public ActionResult Register(RegisterModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                try
                {
                    ...
                    if (model.ObjUser.Id <= 0) //new user
                    { 
                        
                        
                        //setter le nouveau ID
                        model.ObjUser.UserProfileId = WebSecurity.GetUserId(model.UserName);
                        model.ObjUser.Email = model.UserName;
                    }
                     
                    //TODO: Save de notre utilisateur custom
                    Utilisateur.Save(model.ObjUser);
